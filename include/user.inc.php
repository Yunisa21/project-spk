...
class User {
	...
	function insert() {
		...
		$stmt->bindParam(3, $this->rl);
		$stmt->bindParam(4, $this->un);
		$stmt->bindParam(5, $this->pw);
	...
	function getNewID() {
		$query = "SELECT id_pengguna FROM {$this->table_name} ORDER BY id_pengguna DESC LIMIT 1";
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		if ($stmt->rowCount()) {
			$row = $stmt->fetch(PDO::FETCH_ASSOC);
			$pcs = explode("C", $row['id_pengguna']);
			$result = "U".($pcs[1]+1);
		} else {
			$result = "U1";
		}
